        ...
        <form @submit.prevent="handleSubmit" class="px-6 py-4">
          ...
          <div class="mb-4">
            ...
            <div class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-600">
              Cette activité spécifique sera rattachée à l'activité générale actuelle
            </div>
          </div>
...
const resetForm = () => {
  form.titre = ''
  form.activite_generale = props.activiteGeneraleId
  formErrors.value = {}
}
...
const handleSubmit = async () => {
  ...
  try {
    if (import.meta.dev) {
      console.log('🔄 Soumission création activité spécifique:', form.titre)
    }

    const result = await createActiviteSpecifique({
      titre: form.titre,
    ...
    if (result.success) {
      if (import.meta.dev) {
        console.log('🟢 Activité spécifique créée avec succès:', result.data)
      }

      // Émettre l'événement de création avec les données
