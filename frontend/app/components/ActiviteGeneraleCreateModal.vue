...
const handleSubmit = async () => {
  ...
  // Création de l'activité générale
  const result = await createActiviteGenerale({
    ...form.value,
    sous_projet: props.sousProjetId
  })
  ...
  if (result.success) {
    ...
    emit('created', result.activiteGenerale)
...
watch(() => props.isOpen, (newVal) => {
  if (newVal) {
    // Charger les qualiticiens quand le modal s'ouvre
    fetchQualiticiens()
  } else {
