    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      ...
      <div class="mt-3">
        ...
        <div class="text-center">
          ...
          <div v-if="sousProjet" class="text-sm text-gray-500 mb-4">
            <p>Cette action supprimera définitivement le sous-projet :</p>
            <p class="font-medium text-gray-900 mt-2">{{ sousProjet.titre }}</p>
            
            <!-- Informations du sous-projet -->
            <div class="mt-4 text-left bg-gray-50 rounded-md p-3">
              <dl class="space-y-1">
                <div v-if="sousProjet.chef_projet_details">
                  <dt class="text-xs font-medium text-gray-500">Chef de projet :</dt>
                  <dd class="text-sm text-gray-900">
                    {{ sousProjet.chef_projet_details.name }} {{ sousProjet.chef_projet_details.surname }}
                  </dd>
                </div>
                
                <div v-if="sousProjet.ingenieur_travaux_details">
                  <dt class="text-xs font-medium text-gray-500">Ingénieur travaux :</dt>
                  <dd class="text-sm text-gray-900">
...
const handleDelete = async () => {
  if (!props.sousProjet) return

  const result = await deleteSousProjet(props.sousProjet.id)

  if (result.success) {
    // Émettre l'événement avec les données du sous-projet supprimé
    emit('deleted', props.sousProjet)
